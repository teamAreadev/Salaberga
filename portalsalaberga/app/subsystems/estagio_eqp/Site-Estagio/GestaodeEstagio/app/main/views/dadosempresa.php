...
<body class="min-h-screen font-['Roboto'] select-none">
            ...
            <div class="md:hidden header-container">
                ...
                <div class="header-top-row">
                    ...
                    <div class="school-info-mobile">
                        ...
                        <h1 class="page-title-mobile">Dados das Empresas</h1>
                    </div>
                </div>

                <!-- Segunda linha: Botões de ação -->
                <div class="header-actions-row">
                    <a href="../views/relatorios/relatorio.php" class="action-button-mobile" title="Gerar Relatório PDF">
                        <i class="fas fa-file-pdf"></i>
                        <span>PDF</span>
                    </a>
                    <a href="../views/cadastrodaempresa.php" class="action-button-mobile" title="Cadastrar Nova Empresa">
                        <i class="fas fa-plus"></i>
                        <span>Nova</span>
                    </a>
                </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    if ($result > 0) {
                        foreach ($query as $value) {
                            ...
                            echo "<button type='submit' class='text-ceara-orange hover:text-ceara-green p-2 rounded-full transition-colors' aria-label='Editar empresa " . htmlspecialchars($value['nome']) . "'>";
                            echo "<i class='fas fa-edit'></i>";
                            echo "</button>";
                            echo "</form>";
                            echo "<form action='../controllers/Controller-Exclusoes.php' method='POST' style='display: inline;' onsubmit='return confirm(\"Tem certeza que deseja excluir esta empresa?\");'>";
                            echo "<input type='hidden' name='tipo' value='empresa'>";
                            echo "<button type='submit' name='btn-excluir' value='" . htmlspecialchars($value['id']) . "' class='text-red-600 hover:text-red-800 bg-red-50 p-2 rounded-full transition-colors' title='Excluir empresa' aria-label='Excluir empresa'><i class='fas fa-trash'></i></button>";
                            echo "</form>";
                            echo "</div>";
                            echo "</td>";
                            echo "</tr>";
        ...
        <div class="md:hidden">
            ...
            if ($result > 0) {
                foreach ($query as $value) {
                    ...
                    // Header do card com logo e info básica
                    echo "<div class='company-info'>";
                    echo "<img class='company-avatar' src='https://ui-avatars.com/api/?name=" . urlencode($value['nome']) . "' alt='Logo da empresa " . htmlspecialchars($value['nome']) . "'>";
                    echo "<div>";
                    ...
                    // Detalhes da empresa
                    echo "<div class='company-details space-y-2'>";
                    ...
                    echo "<p><i class='fas fa-users'></i>" . htmlspecialchars($value['numero_vagas']) . " vagas disponíveis</p>";
                    echo "</div>";
                    
                    echo "</div>";
                    
                    // Ações do card
                    echo "<div class='card-actions flex justify-end items-center gap-2'>";
                    echo "<form action='../controllers/Controller-botao_acao.php' method='GET' style='display: inline;'>";
                    echo "<input type='hidden' name='btn-editar_empresa' value='" . htmlspecialchars($value['id']) . "'>";
                    echo "<button type='submit' class='text-ceara-orange hover:text-ceara-green p-2 rounded-full transition-colors' aria-label='Editar empresa " . htmlspecialchars($value['nome']) . "'>";
                    echo "<i class='fas fa-edit'></i>";
                ...
                echo "<i class='fas fa-building text-4xl text-gray-300 mb-4'></i>";
                echo "<p class='text-lg text-gray-500'>Nenhuma empresa cadastrada</p>";
                echo "<p class='text-sm text-gray-400'>Clique em 'Nova' para adicionar a primeira empresa</p>";
                echo "</div>";
                echo "</div>";
            }
            ?>
    ...
    <script>
        function showCompanyDetails(company) {
            ...
            console.log('Detalhes da empresa:', company);
        }
