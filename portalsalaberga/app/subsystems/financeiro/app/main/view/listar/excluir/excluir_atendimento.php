...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ids']) && is_array($_POST['ids'])) {
    ...
    $sql = "SELECT caminho_pdf FROM atendimentos WHERE id IN ($ids_str)";
    ...
    if ($result) {
        ...
        $sql = "DELETE FROM atendimentos WHERE id IN ($ids_str)";
        if ($conn->query($sql)) {
            header("Location: ../listar_atendimento.php?success=1");
            exit;
        } else {
            error_log("Erro ao excluir do banco: " . $conn->error);
            header("Location: ../listar_atendimento.php?error=1&msg=" . urlencode($conn->error));
            exit;
        }
    } else {
