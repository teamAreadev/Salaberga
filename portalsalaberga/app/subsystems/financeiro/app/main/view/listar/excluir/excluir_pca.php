...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ids']) && is_array($_POST['ids'])) {
    ...
    $sql = "SELECT caminho_pdf FROM pca WHERE id IN ($ids_str)";
    ...
    if ($result) {
        ...
        $sql = "DELETE FROM pca WHERE id IN ($ids_str)";
        if ($conn->query($sql)) {
            header("Location: ../listar_pca.php?success=1");
            exit;
        } else {
            error_log("Erro ao excluir do banco: " . $conn->error);
            header("Location: ../listar_pca.php?error=1&msg=" . urlencode($conn->error));
            exit;
        }
    } else {
    ...
    header("Location: ../listar_pca.php");
    exit;
}
$conn->close();
?>
