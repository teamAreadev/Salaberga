...
// Variáveis com verificação segura
...
if (!is_dir('declaracoes')) mkdir('declaracoes', 0777, true);

// Nome do PDF
$nome_pdf = 'observancia_' . preg_replace('/[^0-9]/', '', $nup) . '_' . date('Ymd_His') . '.pdf';
$caminho = 'declaracoes/' . $nome_pdf;
...
// Registro no banco
$stmt = $conn->prepare("INSERT INTO observancias (natureza, nup, escola, ordenador, caminho_pdf) VALUES (?, ?, ?, ?, ?)");
$stmt->bind_param("sssss", $natureza, $nup, $escola, $ordenador, $caminho);
$stmt->execute();
...
<head>
    ...
    <title>Declaração Gerada - Sistema Financeiro</title>
    <!-- Bootstrap 5 CSS -->
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Declaração Gerada com Sucesso!</h5>
                </div>
                <div class="modal-body text-center">
                    <i class="fas fa-check-circle success-icon"></i>
                    <p class="success-message">A declaração foi gerada e salva com sucesso.</p>
