...
class qrCode1 extends connect
{
    ...
    public function pdf()
    {
        ...
        // Obter dados enviados via POST ou GET
        $livros = $_POST['livro'] ?? $_POST['livro'] ?? [];
        if (!is_array($livros)) {
            $livros = [$livros]; // Garantir que seja um array
        }
        ...
        foreach ($livros as $livro) {
            // Extrair dados do formato "titulo_id_edicao_numero_estante_prateleira_cativo"
            $livro_array = explode("_", $livro);
            if (count($livro_array) < 7) {
                continue; // Pular se os dados estiverem incompletos
            }

            $titulo_livro = $livro_array[0];
            $id_livro = $livro_array[1];
            $edicao = $livro_array[2];
            $numero = $livro_array[3];
            $estante = $livro_array[4];
            $prateleira = $livro_array[5];
            $cativo = $livro_array[6];

            // Determinar a edição para a URL
            $edicao_url = ($edicao == 'ENI*' || empty($edicao)) ? '0' : $edicao;

            // Dados do QR Code
            $dados = "https://salaberga.com/salaberga/portalsalaberga/app/subsystems/biblioteca/app/main/views/emprestimo/decisao.php?id_livro=" . $id_livro . "_" . $edicao_url . "_" . $numero . "_" . $estante . "_" . $prateleira;
            ...
            $nome_livro = substr(utf8_decode($titulo_livro), 0, 25);
            $pdf->SetXY($current_x, $current_y + $qr_size + 2);
            $pdf->Cell($qr_size, 10, $nome_livro, 0, 0, 'L');

            // Segunda linha: ID, Edição, Número
            $codigo = utf8_decode("Id: " . $id_livro . " | Edição: " . $edicao_url . " | Livro: " . $numero);
            $pdf->SetXY($current_x, $current_y + $qr_size + 10);
            ...
            $pdf->SetXY($current_x, $current_y + $qr_size + 18);
            $pdf->Cell($qr_size, 10, $localizacao, 0, 0, 'L');

            // Quarta linha: Cativo/Não cativo
            $cativo_texto = ($cativo == 0) ? utf8_decode("Não cativo") : utf8_decode("Cativo");
...
// Verificar se há dados enviados via POST ou GET
if (isset($_POST['livro']) && !empty($_POST['livro'])) {
    $qrcode = new qrCode1;
} else {
    header('location:geradorQR_especifico_livro.php');
    exit();
}